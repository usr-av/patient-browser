...
export class PatientDetail extends React.Component {
    ...
    return (
      Promise.resolve({ ...this.state })
        ...
        .then((state) => {
          ...
          return this.query.fetch(server).then((bundle) => {
            state.bundle = bundle;
            return state;
          });
        })
        ...
        .then((state) => {
          ...
          state.hasNext =
            !!state.nextURL ||
            (state.patient ? index < state.bundle.entry.length - 1 : false);
        ...
        .then((state) => {
          return getAllPages({
            url: `${server.url}/Patient/${state.patient.id}/$everything?_count=500`,
          }).then((data) => {
            let groups = {};
            data.forEach((entry) => {
              let resourceType =
  ...
  renderPatient() {
    ...
    let key =
      idLower &&
      Object.keys(this.props.selection).find((k) => k.toLowerCase() == idLower);
            ...
            <div className="patient-row">
              ...
              <div className="col-xs-5 text-right" style={{ paddingRight: 0 }}>
                <button
                  ...
                  onClick={() =>
                    store.dispatch(toggle({ id: this.state.patient.id }))
                  }
                >
                ...
                <div className="row navigator">
                  <div className="col-xs-4 col-sm-4 col-md-3 col-lg-2">
                    ...
                    >
                      ...
                      <b>
                        Prev<span className="hidden-xs">ious Patient</span>
                      </b>
                    </Link>
                  </div>
                  ...
                  <div className="col-xs-4 col-sm-4 col-md-3 col-lg-2 text-right">
                    ...
                    >
                      <b>
                        Next<span className="hidden-xs"> Patient</span>
                      </b>
